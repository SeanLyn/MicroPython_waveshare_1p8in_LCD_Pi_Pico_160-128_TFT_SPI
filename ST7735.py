...
# from https://github.com/boochow/MicroPython-ST7735
# modified from https://github.com/GuyCarver/MicroPython/tree/master/lib
...
#                             0x30, 0x39, 0x3f, 0x00, 0x07, 0x03, 0x10])
  ...
  def draw_mandelbrot(self,spi):
    ...
    for y in range(height):
      ...
      self._setwindowloc((0,y),(127,y))
      self.dc(1)
      self.cs(0)
      spi.write(buf)
      self.cs(1)

  def show_2bit_bmp(self,bmp_file):
    f=open(bmp_file, 'rb')
    if f.read(2) == b'BM':  #header
      bmp_file_size = int.from_bytes(f.read(4), 'little')
      dummy = f.read(4) #file size(4), creator bytes(4)
      ...
      if int.from_bytes(f.read(2), 'little') == 1: #planes must be 1
        ...
        if (depth == 1) and  (compress_method== 0):#compress method == uncompressed
          if height < 0:
            height = -height
            flip = False
          else:
          ...
          rowsize_bytes = 4*int((width * depth+31)/32)

          for row in range(h):
            if flip:
              pos = offset + (height - 1 - row) * rowsize_bytes
            else:
              pos = offset + row * rowsize_bytes
            f.seek(pos)  
            row_pixel_data_hex = f.read(rowsize_bytes)
            for byte_index in range(len(row_pixel_data_hex)):
              current_byte_binary = str("{0:8b}".format(row_pixel_data_hex[byte_index])).replace(" ","0")
              for bit_index in range(len(current_byte_binary)):
                if (current_byte_binary[bit_index]=="1"):
                  self._writedata(b'\xff\xff')
                elif (current_byte_binary[bit_index]=="0"):
                  self._writedata(b'\x00\x00')
                else:
                  pass           
      
  def show_24bit_bmp(self,bmp_file):
    f=open(bmp_file, 'rb')
    if f.read(2) == b'BM':  #header
...
def init_tft():
  # Pins used by SPI driver on RaspBerry Pi Pico and WaveShare 1.8inch TFT LCD
  DC = 8
  CS = 9
  SCK = 10
  MOSI = 11
  ...
  spi = SPI(1,baudrate ,polarity=0, phase=0,sck=Pin(SCK),mosi=Pin(MOSI),miso=None)
